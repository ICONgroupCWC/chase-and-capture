...
def runTest(agent_0_QNET, agent_1_QNET,testEpisodes):
    listofNetNames = [agent_0_QNET,agent_1_QNET]
    steps_history = []
    steps_num = 0
    ...
    listofNets = []
    for netName in listofNetNames:
        net = QNetworkCoordinated(M_AGENTS, P_PREY, 5)
        net.load_state_dict(torch.load(netName))
        net.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
        net.eval()
        listofNets.append(net)



    env = gym.make(SpiderAndFlyEnv)
    
    for epi in range(testEpisodes):
        ...
        while not all(done_n):
            ...
            with ThreadPoolExecutor(max_workers=_n_workers) as executor:
                futures = [
                    ...
                    for agent_i, (obs, net) in enumerate(zip(obs_n, listofNets))
                ]
...
if __name__ == '__main__':
    ...
    for i in range(50):
        epochNum = i*200
        print(f"Loading Model {epochNum}")
        agent_0_QNET = 'artifacts/models/epoch_' + str(epochNum) + '_deter_5050_mode11_agent_0'
        agent_1_QNET = 'artifacts/models/epoch_' + str(epochNum) + '_deter_5050_mode11_agent_1'
        stepsHist = runTest(agent_0_QNET, agent_1_QNET,10)
        print(stepsHist)
        logsDict[epochNum] = stepsHist

        with open("result_autonomous_2Agent_iDQN.json", "w") as json_file:
            json.dump(logsDict, json_file)
